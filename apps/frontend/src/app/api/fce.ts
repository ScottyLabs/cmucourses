...
const fetchFCEInfosByCourseBatcher = memoize(
  (isSignedIn: boolean | undefined, getToken: GetToken) => {
    return create({
      fetcher: async (
        courseIDs: string[]
      ): Promise<{ courseID: string; fces: FCE[] }[]> => {
...
export const useFetchFCEInfosByCourse = (
  courseIDs: string[],
...
const fetchFCEInfosByInstructorBatcher = memoize(
  (isSignedIn: boolean | undefined, getToken: GetToken) => {
    return create({
      fetcher: async (
        instructors: string[]
      ): Promise<{ instructor: string; fces: FCE[] }[]> => {
        const url = `${process.env.NEXT_PUBLIC_BACKEND_URL || ""}/fces`;
        const params = new URLSearchParams();
        ...
        if (isSignedIn && token) {
          ...
          return instructors.map((instructor) => ({
            instructor,
            fces: response.data.filter(
              (fce: FCE) => fce.instructor === instructor
            ),
          }));
        }
        return [];
...
export const useFetchFCEInfosByInstructor = (
  instructor: string,
  isSignedIn: boolean | undefined,
  getToken: GetToken
) => {
