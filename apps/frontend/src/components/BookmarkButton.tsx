...
const BookmarkButton = ({ courseID }: Props) => {
  ...
  const toggleCourseInSchedule = (schedule: UserSchedule) => {
    ...
    if (schedule.courses.includes(courseID)) {
      dispatch(
        userSchedulesSlice.actions.removeCourseFromActiveSchedule(courseID)
      );
    } else {
      ...
      <GetTooltip id={id}>
        <div className="right-0 top-full w-auto min-w-36 bg-white rounded -m-3 py-3 px-1">
          <ul>
            <li
              key="titlle"
              className="relative cursor-pointer select-none pl-3 pr-9 focus:outline-none"
            >
              <b>Add to schedule</b>
            </li>
            <li
              key="saved"
              className="flex relative cursor-pointer select-none pt-2 pl-3 pr-9 focus:outline-none"
            ...
            {Object.values(saved).map((schedule) => (
              <li
                ...
                className={classNames(
                  "flex relative cursor-pointer select-none pt-2 focus:outline-none",
            ...
            <li
              ...
              onClick={() => {
                dispatch(
                  userSchedulesSlice.actions.createSharedSchedule([courseID])
                );
              }}
            >
