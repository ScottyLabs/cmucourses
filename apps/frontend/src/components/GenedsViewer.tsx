...
const GenedsViewer = () => {
  ...
  const {
    ...
    error,
    data: geneds,
  } = useFetchGenedsBySchool(selectedSchool, isSignedIn, getToken);
  ...
  const deleteTag = (tagToDelete: string) => {
    dispatch(
      userSlice.actions.setSelectedTags(
  ...
  useEffect(() => {
    if (geneds && geneds.length > 0) {
      ...
      if (searchQuery.length > 0) {
        mappedGeneds = mappedGeneds.filter(
          (gened) =>
    ...
    <div className="p-3 m-2 bg-white rounded">
      <div className="relative m-2">
        ...
        <Combobox
          value={selectedTags}
          onChange={(payload) =>
            dispatch(userSlice.actions.setSelectedTags(payload))
          }
          multiple
        >
              ...
              <Combobox.Input
                ...
                onKeyDown={(e: React.KeyboardEvent) => {
                  if (
                    ...
                    const tagToAdd = tags.filter((tag) =>
                      tag.includes(tagQuery)
                    )[0];
                    if (tagToAdd && !selectedTags.includes(tagToAdd)) {
                      dispatch(
                        userSlice.actions.setSelectedTags(
                          selectedTags.concat([tagToAdd])
                        )
                      );
                    }
                  }
