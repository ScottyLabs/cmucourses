...
const columns: ColumnDef<FinalExamRow>[] = [
  {
    ...
    cell: ({ row }) => {
      ...
      return (
        <>
          <Link data-tooltip-id={id} href={`/course/${formattedCourseCode}`}>
            {formattedCourseCode}
          </Link>
          <GetTooltip
            id={id}
            children={
              <div>
                <b>{row.original.name as string}</b> <br />
                {row.original.desc as string}
              </div>
            }
          />
        </>
...
const SIO_CAL_EXPORT_URL =
  "https://s3.andrew.cmu.edu/sio/mpa/secure/export/schedule/F25_schedule.ics";
...
export default function FinalsViewer() {
  ...
  const filteredRows = useMemo(() => {
    ...
    return FINALS_ROWS.filter((row) => {
      return (row.course + row.section).toLowerCase().includes(normalized);
    });
  ...
  const handleFileChange = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    file.text().then((text) => {
      ...
      for (const entry of calData[2]) {
        ...
        const section =
          codeSection.length > 1 && codeSection[1] ? codeSection[1] : "";
        newScheduleData.push({ code: String(code), section: String(section) });
      }
      dispatch(
        finalsSlice.actions.setOwnCourses(
          newScheduleData.map((c) => c.code + c.section)
        )
      );
    });
  };
  ...
  return (
    <>
          ...
          {!uploaded() && (
            <>
              <p className="text-sm text-gray-500">
                1: Download{" "}
                <Link href={SIO_CAL_EXPORT_URL} openInNewTab>
                  Calendar Export
                </Link>{" "}
                from SIO.
              </p>
            ...
            {uploaded() && (
              <div className="flex items-center gap-2">
                <input
                  ...
                  checked={showOwn}
                  onChange={(e) =>
                    dispatch(finalsSlice.actions.setShowOwn(e.target.checked))
                  }
                />
                <label
                  htmlFor="show-own-checkbox"
      ...
      <div className="mx-auto w-full px-4 pb-12 pt-6">
        <Card>
          <div
            className={classNames("flex flex-col gap-6", "transition-colors")}
          >
